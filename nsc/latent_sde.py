...
class LatentSDE(torchsde.SDEIto):
    ...
    def sample_p(self, ts, batch_size, eps=None, bm=None): # Prior: We assume that z_0 \sim N(mu_p, std_p)
        eps = torch.randn(batch_size, 1).to(self.py0_mean) if eps is None else eps
        y0 = self.py0_mean + eps * self.py0_std
        return sdeint_fn(self, y0, ts, bm=bm, method='srk', dt=args.dt, names={'drift': 'h'})

    def sample_q(self, ts, batch_size, eps=None, bm=None): # We assume that z_0|x_0 \sim N(mu_q, std_q)
...
def main():
    ...
    pbar = tqdm.tqdm(range(args.train_iters), desc="Training", ncols=120)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train-dir', type=str, default='data_processed/latent/')
